...
def get_token_name_by_url_by_datas( json_dict:dict )->dict:
    dict_token = dict()
    dict_text = dict()
    for url in json_dict.keys():
        for data in json_dict[ url ].keys():
            dict_token[ url + "-" + data ] = set()
            dict_text[ url + "-" + data ] = list()
    ...
    for url in json_dict.keys():
        for data in json_dict[ url ].keys():
            ...
            for ele in json_dict[ url ][ data ]:
                if len( ele ) > 50:
                    list_apo.append( ele )
                    dict_text[ url + "-" + data ].append( ele )
            lib_token_str.get_names_list( list_apo , dict_token[ url + "-" + data ] )
    return dict_token , dict_text
...
if __name__ == "__main__":
    ...
    PATH_BY_URLS_BY_DATA = '../data/json_refined/most_cited/by_url_by_data/'
    ...
    if len( sys.argv ) <= 1:
        ...
        dict_by_url_by_datas = count_by_token_name( dict_json , get_token_name_by_url_by_datas )
        for key in dict_by_url_by_datas.keys():
            dict_json = lib_json_utils.parse_key_value_to_label_value (  dict_by_url_by_datas[ key ] ) 
            arq = open( PATH_BY_URLS_BY_DATA + url_to_path( key ) + '.json' , 'w')
            arq.write( json.dumps( dict_json , indent = 4) )
            arq.close()
        print("[ADD  ] Most Cited, generated by  urls by data " + str( len( dict_by_url_by_datas ) ) )
